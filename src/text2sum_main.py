...
from src.seq2seq_model import Text2SumModel
from src.text2sum_iterator import Txt2SumIterator
from src.text2sum_updater import Text2SumUpdater
import os
...
def main():
	...
	train_text, train_sum, text_dict, sum_dict = \
		load_data(*map(lambda dir: os.path.join(projct_path, dir), (args.text_source, args.sum_target, args.text_vocab,
		                                                            args.sum_vocab)),
		          max_doc_vocab=args.text_vocab_size, max_sum_vocab=args.sum_vocab_size)
	...
	val_text, val_sum = \
		load_valid_data(*map(lambda dir: os.path.join(projct_path, dir),
		                     (args.val_text_source, args.val_sum_target)), doc_dict=text_dict, sum_dict=sum_dict)
	...
	def validate(trainer):
		...
		bilstm_model.validate(encoder_inputs, decoder_target)
	...
	trainer.extend(extensions.PrintReport(
		['epoch','iteration', 'main/train_loss', 'main/train_perp', 'main/val_correlation',  'main/val_perp', 'elapsed_time']),
		trigger=(args.log_interval, 'iteration'))
